...
const $DataSterling = require('../models/Sterling');
...
exports.insert = async (req, res) => {
    try {
        ...
        const jsonSterling = await csv({
            flatKeys: true,
        }).fromFile(csvFilePath);

        await $DataSterling.create({ dataExcel: jsonSterling });
...
exports.index = async (req, res) => {
    try {
        ...
        jsonSterling.forEach(async (i) => {
            await $DataSterling.create({
                no_loc_vta: i.no_loc_vta,
                mesaevento: i.mesaevento,
                fecha_em: i.fecha_em,
                remision: i.remision,
                edo_cabe: i.edo_cabe,
                fecha_prom: i.fecha_prom,
                fecha_prom_1: i.fecha_prom_1,
                a: i.a
            });
        });

        // await $DataSterling.create({sterling: a});
        ...
        // for (let x = 0; x < logsLength; x++) {
            ...
            //     if ( dataLogs[x].fecConsulta === dataSterling[y].fecha_em
                    ...
                    // excelMaster.push({
                        ...
                        // mesaevento: dataSterling[y].fecha_em
